...
var ASM_CONSTS = {
  5757756: () => { Module['emscripten_get_now_backup'] = performance.now; },  
 5757811: ($0) => { performance.now = function() { return $0; }; },  
 5757859: ($0) => { performance.now = function() { return $0; }; },  
 5757907: () => { performance.now = Module['emscripten_get_now_backup']; }
};
...
// end include: preamble.js
  ...
  function _CheckWalletConnection() {
          const isConnected = tonConnectUI.wallet;
          if (isConnected) {
              const walletAddress = tonConnectUI.wallet.account.address;
              if (window.unityInstance) {
                  window.unityInstance.SendMessage("TonWalletManager", "OnCheckWalletConnection", walletAddress);
              }
          } else {
              window.unityInstance.SendMessage("TonWalletManager", "OnWalletDisconnected");
          }
      }

  function _ClipboardWriter(newClipText) {
        try {
          var clipText = UTF8ToString(newClipText);
        console.log(newClipText);
  ...
  function _ConnectTonWallet() {
          tonConnectUI.connectWallet().then(() => {
              var walletAddress = tonConnectUI.wallet.account.address;
              
              console.error("walletAddress:", walletAddress);
              if (window.unityInstance) {
                  window.unityInstance.SendMessage("TonWalletManager", "OnWalletConnected", walletAddress);
              }
          }).catch((err) => {
  ...
  async function _CreateTonWallet(publicKey) {
        if (window.tonweb) {
          const wallet = window.tonweb.wallet.create({ publicKey: UTF8ToString(publicKey) });
          window.tonWallet = wallet;
          const address = await wallet.getAddress();
          const nonBounceableAddress = address.toString(true, true, false);
          if (window.unityInstance) {
            window.unityInstance.SendMessage("TelegramController", "SetTonWalletAddress", nonBounceableAddress);
          }
        }
      }

  async function _DeployTonWallet(secretKey) {
        if (window.tonWallet) {
          await window.tonWallet.deploy(UTF8ToString(secretKey)).send();
        }
      }

  function _DetectDeviceRuntime() {
          var userAgent = navigator.userAgent || navigator.vendor || window.opera;
  
          if (/android/i.test(userAgent)) {
              return 1; // Return 1 for Android
          }
  
          if (/iPad|iPhone|iPod/.test(userAgent) && !window.MSStream) {
              return 2; // Return 2 for iOS
          }
  
          if (/Windows/.test(userAgent)) {
              return 3; // Return 3 for Windows
          }
  
          return 0; // Default return value for other platforms
      }

  function _DisconnectTonWallet() {
  ...
  function _GetTonAddress(userFriendly) {
          const wallet = tonConnectUI.wallet;
          if (wallet) {
              const address = wallet.account.address;
              const chain = wallet.account.chain;
              const userFriendlyAddress = userFriendly
                  ? TON_CONNECT_UI.toUserFriendlyAddress(address, chain === TON_CONNECT_UI.CHAIN.TESTNET)
                  : address;
              window.unityInstance.SendMessage("TonWalletManager", "OnTonAddressFetched", userFriendlyAddress);
          }
      }

  async function _GetTonBalance() {
        if (window.tonWallet) {
          const address = await window.tonWallet.getAddress();
          const balance = await window.tonweb.getBalance(address);
          if (window.unityInstance) {
            window.unityInstance.SendMessage("TelegramController", "SetTonWalletBalance", balance);
          }
        }
      }

  async function _GetTonTransactions() {
        if (window.tonWallet) {
          const address = await window.tonWallet.getAddress();
          const transactions = await window.tonweb.getTransactions(address);
          if (window.unityInstance) {
            window.unityInstance.SendMessage("TelegramController", "SetTonTransactions", JSON.stringify(transactions));
          }
        }
      }

  function _GetTransactionHash(bocString) {
          
          console.error("bocString:", bocString);
          const bocCell = tonweb.boc.Cell.oneFromBoc(tonweb.utils.base64ToBytes(UTF8ToString(bocString)));
          bocCell.hash().then((msgHash) => {
              const transactionHash = tonweb.utils.bytesToBase64(msgHash);
              window.unityInstance.SendMessage("TonWalletManager", "OnTransactionHashFetched", transactionHash);
          }).catch((err) => {
              console.error("Failed to get transaction hash:", err);
          });
      }

  function _HapticFeedback(level) {
      if (window && window.Telegram && window.Telegram.WebApp) {
  ...
  function _HideMainButton() {
      if (window && window.Telegram && window.Telegram.WebApp) {
        window.Telegram.WebApp.MainButton.hide();
      }
    }

  function _InitializeTonWeb(apiKey, isTestnet) {
        const providerUrl = isTestnet 
          ? 'https://testnet.toncenter.com/api/v2/jsonRPC'
          : 'https://toncenter.com/api/v2/jsonRPC';
        const provider = new TonWeb.HttpProvider(providerUrl, { apiKey: UTF8ToString(apiKey) });
  ...
  function _MainButtonHideProgress() {
      if (window && window.Telegram && window.Telegram.WebApp) {
        window.Telegram.WebApp.MainButton.hideProgress();
      }
    }

  function _MainButtonShowProgress() {
      if (window && window.Telegram && window.Telegram.WebApp) {
  ...
  function _RequestUserInItData() {
      if (window.unityInstance) {
        window.unityInstance.SendMessage("TelegramController", "SetWebAppInItData", JSON.stringify(window.Telegram.WebApp.initData));
      }
    }

  function _RequestUserParamsData() {
      if (window.unityInstance) {
        window.unityInstance.SendMessage("TelegramController", "SetWebAppUserParams", JSON.stringify(window.Telegram.WebApp.initDataUnsafe));
      }
    }

  function _SendTonTransaction(recipient, amountInNanoTons) {
          
          console.error("recipient:", UTF8ToString(recipient));
          console.error("amountInNanoTons:", UTF8ToString(amountInNanoTons));
          const transaction = {
              validUntil: Math.floor(Date.now() / 1000) + 600,
              messages: [{ address: UTF8ToString(recipient), amount: UTF8ToString(amountInNanoTons) }]
          };
  
          tonConnectUI.sendTransaction(transaction).then((result) => {
              
              console.error("result.boc:", result.boc);
              if (window.unityInstance) {
                  window.unityInstance.SendMessage("TonWalletManager", "OnTransactionSuccess", result.boc);
              }
          }).catch((err) => {
              
              console.error("Transaction failed:", err.message);  // Log error message
              console.error("Full error object:", err);  // Log full error object for debugging
              
              if (window.unityInstance) {
                  window.unityInstance.SendMessage("TonWalletManager", "OnTransactionFailed", err.message);
              }
          });
      }
  ...
  function _ShowScanQrPopup(text) {
      if (window && window.Telegram && window.Telegram.WebApp) {
  ...
  async function _TransferTon(secretKey, toAddress, amountNano, payload) {
        if (window.tonWallet) {
          const seqno = await window.tonWallet.methods.seqno().call();
          const transferParams = {
            secretKey: UTF8ToString(secretKey),
            toAddress: UTF8ToString(toAddress),
            amount: TonWeb.utils.toNano(parseFloat(UTF8ToString(amountNano))),
            seqno: seqno,
            payload: UTF8ToString(payload),
            sendMode: 3
          };
          const fee = await window.tonWallet.methods.transfer(transferParams).estimateFee();
          await window.tonWallet.methods.transfer(transferParams).send();
        }
      }
  ...
  function __GetDynamicMemorySize()
      {
  ...
  function _copyTextToClipboard(text) {
          ...
          navigator.clipboard.writeText(str).then(function() {
              console.error("Text copied to clipboard: " + str);
          }, function(err) {
...
var wasmImports = {
  "CheckWalletConnection": _CheckWalletConnection,
  "ClipboardWriter": _ClipboardWriter,
  "Close": _Close,
  "CloseScanQrPopup": _CloseScanQrPopup,
  "ConnectTonWallet": _ConnectTonWallet,
  "CreateTonWallet": _CreateTonWallet,
  "DeployTonWallet": _DeployTonWallet,
  "DetectDeviceRuntime": _DetectDeviceRuntime,
  "DisconnectTonWallet": _DisconnectTonWallet,
  "Expand": _Expand,
  "GetJSLoadTimeInfo": _GetJSLoadTimeInfo,
  "GetJSMemoryInfo": _GetJSMemoryInfo,
  "GetTonAddress": _GetTonAddress,
  "GetTonBalance": _GetTonBalance,
  "GetTonTransactions": _GetTonTransactions,
  "GetTransactionHash": _GetTransactionHash,
  "HapticFeedback": _HapticFeedback,
  "Hello": _Hello,
  "HideBackButton": _HideBackButton,
  "HideMainButton": _HideMainButton,
  "InitializeTonWeb": _InitializeTonWeb,
  ...
  "MainButtonHideProgress": _MainButtonHideProgress,
  "MainButtonShowProgress": _MainButtonShowProgress,
  "OpenURL": _OpenURL,
  "OpenURLblank": _OpenURLblank,
  "Ready": _Ready,
  "RequestThemeParams": _RequestThemeParams,
  "RequestUserData": _RequestUserData,
  "RequestUserInItData": _RequestUserInItData,
  "RequestUserParamsData": _RequestUserParamsData,
  "SendTonTransaction": _SendTonTransaction,
  "ShowAlert": _ShowAlert,
  "ShowBackButton": _ShowBackButton,
  "ShowMainButton": _ShowMainButton,
  "ShowScanQrPopup": _ShowScanQrPopup,
  "ShowShareJoinCode": _ShowShareJoinCode,
  "TransferTon": _TransferTon,
  "_AddFpsTrackingEvent": __AddFpsTrackingEvent,
  "_AddTimeTrackingEvent": __AddTimeTrackingEvent,
  "_GetDynamicMemorySize": __GetDynamicMemorySize,
